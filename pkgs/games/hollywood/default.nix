{ stdenv, fetchgit, makeWrapper, apg, bash, bmon, byobu, ccze, cmatrix, htop, mplayer, openssh, tmux, tree }:
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    wrapProgram $out/bin/hollywood \
      --suffix-each PATH : "${apg}/bin ${bmon}/bin ${byobu}/bin ${ccze}/bin ${cmatrix}/bin ${htop}/bin ${mplayer}/bin ${openssh}/bin ${tmux}/bin ${tree}/bin"
  '';
}
